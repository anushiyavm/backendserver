...
export let loginUser = async (req, res) => {
    try {
        ...
        let token =
            jwt.sign({ id: isExist._id }, process.env.secret_key, { expiresIn: "7d" });
        return res.status(200).json({ response: "login success", ok: true, token });
    } catch (error) {
        console.log(error);
        return res.status(500).json({ response: "server error", ok: false });
    }
...
export let forgotPass = async (req, res) => {
    ...
    try {
        ...
        let mailOption = {
            ...
            <a href=https://luxury-tarsier-086fb0.netlify.app/reset-password/${isExist._id}/${token} style="display: inline-block; background-color: green; color: white; padding: 10px
20px; text-align: center; text-decoration: none; border-radius: 5px; cursor: pointer;">Click here </a>
